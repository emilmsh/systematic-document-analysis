...
def interrupted_install_recovery(base, env):
    ...
    blocked = subprocess.run([sys.executable,'-X','utf8',str(ROOT/'bin/installer.py'),'begge',
    ...
    codex=json.loads(subprocess.check_output(['codex','plugin','list','--marketplace',f'{NAME}-local','--json'],env=env,encoding='utf-8'))
