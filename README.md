...
The installer keeps separate copies in `%LOCALAPPDATA%/systematic-document-analysis/plugins/<app>/systematic-document-analysis`, registered under `systematic-document-analysis-local`. Keep these installed copies; the extracted download can be removed. It recognises existing versions, offers an explicit source switch when paths differ, and preserves a backup when replacing files. Close plugin sessions before updating.

Run **update.cmd** in the installed folder to check for releases, update now, or choose **notify only** (default), **automatic**, or **off**. Checks happen at most daily when a managed plugin session starts. Automatic installation waits until other sessions are closed. This private repository requires an already signed-in GitHub CLI with repository access; a shared release ZIP remains usable without GitHub access. See [installation and updates](docs/UPDATES.md).
