...
server = UpdatingMCPServer(
    ...
    version=VERSJON,
    instructions=(
        "Systematic Document Analysis: a controlled for-loop over files. One agreed task, one independent CLI/API worker per file. "
        "The task determines the result; readable Markdown is the default, with optional output_schema and quote_checks. "
        "Do not force general tasks into classification criteria or a fixed workbook. Large files use task-aware map-reduce. "
        "Respond in the user's language. Check shared startup prerequisites: service/tools, sign-in, repeatable task, source selection, reader/settings and plan approval. "
        ...
        "Task definition, extraction checks and plan preparation may continue. Recheck show_setup after user login; "
        ...
        "propose a repeatable instruction, useful deliverable/checks and routine settings; ask only about material ambiguity. "
        ...
        "Present a concise plain-language plan with file scope, task/result contract, uncertainty rules, output location, reader/settings/recipient "
        ...
        "is required before reader execution; reuse valid authorization and never invent the responsible person's name. Pilots are optional. "
        "Prefer the English tools: show_setup → create_project → import_documents → create_analysis → show_plan "
        "→ add_runs → show_input_package → approve_plan → start_runs → show_status → show_run → export_results. "
        "record_review is for actual human decisions, never a mandatory step or a machine approval. "
        "Choose a visible project directory with create_project(directory=...) or set_project_directory; show the returned path. "
        "Plans and input previews are saved there before execution. Export actual per-file results and their audit trail. "
        "The host may prepare the task and derive reports, tables or cross-file synthesis afterward, with originating run/attempt references. "
        "Legacy criteria_file plans and their workbook remain supported; general tasks require no criteria_file. "
        "Choose language en or nb explicitly in the plan. Use the user's own documents; simulation is optional. "
        "Never translate source quotes, result keys or answer labels. Explain legacy diagnostics/status codes in the user's language. "
