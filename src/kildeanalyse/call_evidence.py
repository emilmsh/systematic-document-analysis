"""Read-only evidence for one worker call per attempt; missing telemetry stays unknown."""
...
def call_records(attempt: dict, document_name: str = '') -> list[dict]:
    ...
    # An active/not-yet-dispatched attempt has no completed call evidence yet.
    if not info and not manifest.get('avsluttet'):
        return []
    params = manifest.get('kjoreparametre') or {}
    motor = manifest.get('motorinfo') or {}
    use = (manifest.get('forbruk') or {}).get('usage') or {}
    root = Path(attempt.get('input_sti') or '.')
    ...
    return [{
        ...
        'document_name': document_name, 'call_index': 1, 'stage': 'single',
        'reader': attempt.get('motor') or manifest.get('motor'),
        'simulated': bool(attempt.get('simulert', manifest.get('simulert', False))),
        'status': ('not_dispatched' if manifest.get('dispatched') is False else
                   'cancelled' if manifest.get('avbrutt') else 'failed' if manifest.get('feil') else
                   manifest.get('status') or 'reply_received'),
        'session_id': manifest.get('sesjon_id'), 'request_id': motor.get('request_id'),
        ...
        'reported_model': reported(manifest.get('modell_rapportert')),
        ...
        'started_at': manifest.get('started_at') or manifest.get('startet'),
        'finished_at': manifest.get('finished_at') or manifest.get('avsluttet'),
        'duration_seconds': motor.get('varighet_sek', manifest.get('duration_seconds')),
        ...
        'warnings': warnings, 'error': manifest.get('feil'),
