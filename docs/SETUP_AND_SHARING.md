...
For an existing installation, version checks and source selection happen before files are replaced. Claude Code and Codex then update the plugin from its release channel; `installer.cmd update` checks, updates now or turns automatic updates off. See [installation and updates](UPDATES.md) for source conflicts, backups, GitHub access and command-line options.
