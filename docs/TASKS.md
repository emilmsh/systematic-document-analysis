...
The first workbook sheet, Results, has exactly one row per document by default, including failed, rejected, empty and unstarted runs. Variables from valid current responses retain their types; invalid or rejected responses have blank variables and their reasons in auxiliary sheets. Missing values are not zeros. Document names and status lead the default view; Errors and notes appears when relevant. Telemetry and variable definitions remain in the documentation ZIP. Identical schemas share columns; different definitions remain separate. Use row_scope="runs" for a workbook with all runs, telemetry and variable definitions.
